...
// Serve game page and player data
app.get('/taktik-tablo', (req, res) => {
  res.sendFile(path.join(__dirname, 'taktik-tablo.html'));
});
app.get('/data/players.json', (req, res) => {
  res.sendFile(path.join(__dirname, 'data', 'players.json'));
});

// ---- WebSocket multiplayer for Taktik Tablo ----
const { WebSocketServer } = require('ws');
const http = require('http');
const httpServer = http.createServer(app);
const wss = new WebSocketServer({ server: httpServer, path: '/ws/tablo' });

const tabloRooms = new Map(); // code -> { players: [{ws, name, symbol}], board: {rows, cols} | null }

function genRoomBoard() {
  const CLUBS = ["Trabzonspor","Galatasaray","Fenerbahçe","Beşiktaş","İstanbul Başakşehir","FC Barcelona","Real Madrid","Atletico Madrid","Manchester United","Manchester City","Liverpool","Arsenal","Chelsea","Bayern Münih","Borussia Dortmund","Paris Saint-Germain","Juventus","AC Milan","Inter Milan","Ajax","Porto"];
  const LEAGUES = ["Süper Lig","La Liga","Premier League","Bundesliga","Ligue 1","Serie A","Eredivisie"];
  const pool = [
    ...CLUBS.map(c => ({ type: 'club', name: c })),
    ...LEAGUES.map(l => ({ type: 'league', name: l })),
  ];
  const shuffle = arr => { for (let i = arr.length-1; i > 0; i--) { const j = Math.floor(Math.random()*(i+1)); [arr[i],arr[j]]=[arr[j],arr[i]]; } return arr; };
  const picks = shuffle([...pool]).slice(0, 6);
  return { rows: picks.slice(0, 3), cols: picks.slice(3, 6) };
}
...
wss.on('connection', ws => {
  ...
  ws.on('message', raw => {
    ...
    if (msg.type === 'join') {
      ...
      if (!tabloRooms.has(code)) {
        tabloRooms.set(code, { players: [], board: null, turn: 'X' });
      }
      const room = tabloRooms.get(code);
      if (room.players.length >= 2) { ws.send(JSON.stringify({ type: 'error', msg: 'Oda dolu' })); return; }
      mySymbol = room.players.length === 0 ? 'X' : 'O';
      room.players.push({ ws, name: msg.name, symbol: mySymbol });
      myRoom = room;
      ws.send(JSON.stringify({ type: 'joined', symbol: mySymbol }));

      if (room.players.length === 2) {
        room.board = genRoomBoard();
        broadcast(room, {
          type: 'start',
  ...
  ws.on('close', () => {
    if (myRoom) {
      myRoom.players = myRoom.players.filter(p => p.ws !== ws);
      broadcast(myRoom, { type: 'opponent_left' });
      if (myRoom.players.length === 0) {
        // cleanup room
        for (const [k, v] of tabloRooms) { if (v === myRoom) { tabloRooms.delete(k); break; } }
      }
    }
  });
});

httpServer.listen(3001, '127.0.0.1', () => console.log('API server running on :3001'));
